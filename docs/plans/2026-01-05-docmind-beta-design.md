...
// Mode precedence: If `conversation` exists, use `conversation.mode` and ignore
// top-level `mode`. Top-level `mode` is only used for the first message in a thread.
```
...
```typescript
// src/background/index.ts (runs once on install/update via onInstalled)
...
Popup doesn't know `tabId` (it's an extension page, not a tab). Background tracks `lastPendingTabId` set by the context menu click.
...
// Context menu registration (runs once on install/update)
browser.runtime.onInstalled.addListener(() => {
  browser.contextMenus.create({
    id: 'ask-docmind',
    title: 'Ask DocMind about this',
...
browser.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'ask-docmind' && info.selectionText && tab?.id) {
    ...
    // Best-effort popup open (MV3 only, may fail on some browsers/contexts)
...
if (message.type === 'GET_PENDING_CONTEXT') {
  // Use lastPendingTabId since popup doesn't have sender.tab
...
- JSON extraction uses `\{[\s\S]*\}` regex as a simple approach; implementation should prefer fenced JSON blocks or balanced brace extraction to avoid greedy matching issues with multiple JSON objects
